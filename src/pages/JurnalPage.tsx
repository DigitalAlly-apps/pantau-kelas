  ...
  useEffect(() => {
    const checkScheduledNotifications = () => {
      ...
      activeCases.forEach(item => {
        if (!item.tanggalPemanggilan || !item.waktuPemanggilan) return;
        const schedule = `${item.tanggalPemanggilan}T${item.waktuPemanggilan}`;
        if (schedule > currentMinute) return;

        const notificationKey = `jg_pemanggilan_notif_${item.id}_${schedule}`;
        if (localStorage.getItem(notificationKey)) return;
        localStorage.setItem(notificationKey, '1');

        const message = `Waktu pemanggilan ${item.studentName} (${item.category})`;
        if ('Notification' in window && Notification.permission === 'granted') {
          new Notification('Pengingat Jurnal Guru', { body: message, icon: '/icon-192.png' });
        } else {
          showToast(`🔔 ${message}`);
        }
      });
    ...
    const timer = window.setInterval(checkScheduledNotifications, 30_000);
    return () => window.clearInterval(timer);
  }, [activeCases, showToast]);
  const save = () => {
